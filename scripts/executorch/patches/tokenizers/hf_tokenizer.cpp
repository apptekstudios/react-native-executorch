...
Result<std::string> HFTokenizer::decode(const std::vector<uint64_t> &tokens,
...
Error HFTokenizer::_encode(const std::string &input, std::vector<uint64_t> &ret,
...
HFTokenizer::_decode(const std::vector<std::string> &pieces) const {
  if (_decoder) {
...
Result<std::vector<uint64_t>>
HFTokenizer::byte_pair_encode_(const std::string &piece,
...
Error HFTokenizer::parse_merges(const json &parsed_json) {
  // BPE is the only tokenizer model with a `merges` table. WordPiece /
  // Unigram / WordLevel models legitimately omit it; treating that as a
  // load failure stops perfectly valid BERT-family tokenizers from loading.
  // Skip cleanly when the field is absent or when the model self-identifies
  // as non-BPE.
  if (parsed_json.contains("model") && parsed_json.at("model").is_object()) {
    const auto &model_json = parsed_json.at("model");
    if (model_json.contains("type") && model_json.at("type").is_string()) {
      const std::string model_type = model_json.at("type").get<std::string>();
      if (model_type != "BPE") {
        return Error::Ok;
      }
    }
...
Error HFTokenizer::setup_special_token_ids(
    const std::string & /*path*/, const json &parsed_json,
  ...
  auto set_special = [&](const std::string &token_str, uint64_t &target_id,
                         bool &found_flag) {
    if (!token_str.empty()) {
  ...
  if (!bos_found || !eos_found) {
    ...
    for (size_t i = 0; i < special_token_map_->size(); ++i) {
      ...
      if (!bos_found && (token.find("bos") != std::string::npos ||
                         token.find("begin") != std::string::npos))
        bos_c.push_back(token);
      if (!eos_found && (token.find("eos") != std::string::npos ||
