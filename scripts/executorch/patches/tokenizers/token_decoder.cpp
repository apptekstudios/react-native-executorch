...
TokenDecoderConfig &TokenDecoderConfig::parse_json(const json &json_config) {
  ...
  if (type == "ByteLevel") {
    ...
    wordpiece_prefix = json_config.value("prefix", std::string("##"));
...
ByteLevelTokenDecoder::decode(const std::vector<std::string> &tokens) const {
  ...
  for (const auto &token : tokens) {
    ...
    if (all_chars_are_bytes) {
      all_bytes.insert(all_bytes.end(), current_token_bytes.begin(),
...
ReplaceTokenDecoder::ReplaceTokenDecoder(const std::string &pattern,
                                         const std::string &content)
    : pattern_(pattern), content_(content) {}

std::vector<std::string>
ReplaceTokenDecoder::decode(const std::vector<std::string> &tokens) const {
...
std::vector<std::string>
ByteFallbackTokenDecoder::decode(const std::vector<std::string> &tokens) const {
  std::vector<std::string> decoded_tokens;
  decoded_tokens.reserve(tokens.size());
  for (const auto &token : tokens) {
...
StripTokenDecoder::StripTokenDecoder(const std::string &content_str,
...
std::vector<std::string>
StripTokenDecoder::decode(const std::vector<std::string> &tokens) const {
  std::vector<std::string> decoded_tokens;
  decoded_tokens.reserve(tokens.size());
...
//  FuseTokenDecoder ///////////////////////////////////////////////////////
std::vector<std::string>
FuseTokenDecoder::decode(const std::vector<std::string> &tokens) const {
...
std::vector<std::string>
SequenceTokenDecoder::decode(const std::vector<std::string> &tokens) const {
...
// WordPieceTokenDecoder //////////////////////////////////////////////////////
// Port of huggingface/tokenizers src/decoders/wordpiece.rs (decode_chain).

namespace {

// Mirror of `wordpiece_cleanup` in HF Rust: collapse the spaces that
// whitespace-tokenisation inserted around punctuation and common
// contractions.
inline std::string wordpiece_cleanup(std::string s) {
  static const std::pair<std::string, std::string> kRules[] = {
      {" .", "."},   {" ?", "?"},     {" !", "!"},     {" ,", ","},
...
std::vector<std::string>
WordPieceTokenDecoder::decode(const std::vector<std::string> &tokens) const {
  std::vector<std::string> out;
  out.reserve(tokens.size());
  for (size_t i = 0; i < tokens.size(); ++i) {
    std::string token = tokens[i];
    if (i != 0) {
      if (!prefix_.empty() && token.compare(0, prefix_.size(), prefix_) == 0) {
        token.erase(0, prefix_.size());
      } else {
        token = " " + token;
      }
    }
    if (cleanup_) {
      token = wordpiece_cleanup(std::move(token));
    }
